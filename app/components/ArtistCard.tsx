...
export default function ArtistCard({ profiles }: { profiles: any[] }) {
  ...
  const getTelegramLink = (tg: string) => {
    ...
    if (trimmed.startsWith('http') || trimmed.startsWith('t.me')) {
      return trimmed;
    }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-8 z-20">
        ...
        <div className="mt-4 flex flex-wrap gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-y-2 group-hover:translate-y-0">
          {current.instagram && (
            <a href={current.instagram} target="_blank" rel="noopener noreferrer" className="text-[10px] uppercase font-bold text-zinc-400 hover:text-white">Instagram</a>
          )}
          {current.spotify && (
            <a href={current.spotify} target="_blank" rel="noopener noreferrer" className="text-[10px] uppercase font-bold text-zinc-400 hover:text-white">Spotify</a>
          )}

          {/* ВЫВОД ТЕЛЕГРАМА */}
          {current.telegram && (
            <a
              href={getTelegramLink(current.telegram)}
              target="_blank"
              rel="noopener noreferrer"
              onClick={(e) => e.stopPropagation()} // чтобы при клике на ссылку не листалась карусель
